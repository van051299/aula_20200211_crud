...
if($_POST){
  ...
  $preco = $_POST["preco"];
	...
	if(mysqli_num_rows($resultado) > 0){
		...
		$sql = "INSERT INTO ARTISTA VALUES(NULL, '$nome', '$login', '$senha', '$instrumento',
     '$idade','$disponivel', '$cidade', '$estado', '$contato', '$viagem', '$preco')";

		if(mysqli_query($conexao, $sql))	echo "<script>alert('Cadastro realizado com sucesso!')</script>";
		else die("Falha no Cadastro!");
	}
}
...
<html lang="pt-br">
  ...
  <body>
    ...
    <div class="container rounded " style="background-color: #f5f5f5; padding-bottom: 35px; padding-top: 35px;">
      ...
      <div class="col-4 p-0">
        ...
        <div class="row">
          ...
          <div class="col-2 p-0">
            ...
            <input type="radio"  name="viagem" value="nao" id="viagem" placeholder="" checked>Não 
            <input type="radio"  name="viagem" value="sim" id="viagem" placeholder="" >Sim
          </div>
          <div class="col-2 p-0">
            <label for="disponivel">Disponível para Shows?</label><br>
            <input type="radio"  name="disponivel" value="nao" id="disponivel" placeholder="" checked>Não 
      ...
      </div> <!--FECHAMENTO DO CONTEINER DO CADASTRo-->
  </form>
